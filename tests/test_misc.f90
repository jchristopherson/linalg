...
! ------------------------------------------------------------------------------
    function test_diagonal_mtx_mult() result(rst)
        ...
        ! Test
        if (.not.is_mtx_equal(ans1, c1, REAL64_TOL)) then
            rst = .false.
...
! ------------------------------------------------------------------------------
    function test_rank1_update() result(rst)
        ...
        ! Compare the results
        if (.not.is_mtx_equal(a1, b, REAL64_TOL)) then
...
! ------------------------------------------------------------------------------
    function test_tri_mtx_mult_1() result(rst)
        ...
        if (.not.is_mtx_equal(b, bans, REAL64_TOL)) then
            check = .false.
...
! ------------------------------------------------------------------------------
    ...
    function test_mtx_mult_1() result(rst)
        ...
        if (.not.is_mtx_equal(c, ans, REAL64_TOL)) then
            rst = .false.
...
! ------------------------------------------------------------------------------
    function test_tri_mtx_solve_1() result(rst)
        ...
        ! Verify that A * X1 = B1
        check1 = matmul(a, x1)
        if (.not.is_mtx_equal(b1, check1, REAL64_TOL)) then
            rst = .false.
